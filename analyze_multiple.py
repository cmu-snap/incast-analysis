...
def load_batch(exp_dirs):
    ...
    if PARALLEL:
        ...
        exp_to_data = {
            exp_dir: analysis.get_all_metrics_for_exp(exp_dir)
...
# %%
none_sweep_dir = "/data_hdd/ccanel/incast/incast_sweep_none/background-senders"
none_sweep_exp_to_data = load_sweep(none_sweep_dir)

# %%
static_sweep_dir = "/data_hdd/ccanel/incast/incast_sweep_static/background-senders"
static_sweep_exp_to_data = load_sweep(static_sweep_dir)

# %%
static_sweep_exp_to_data_filtered = {exp: data for exp, data in static_sweep_exp_to_data.items() if data["config"]["numBurstSenders"] == 50}
...
def graph_avg_queue_depth(exp_to_data, sweep_dir, key, key_modifier, xlabel):
    ...
    fig, axes = analysis.get_axes(rows=1, width=6, cols=1)
    ax = axes[0]
...
# %%
graph_avg_queue_depth(static_sweep_exp_to_data_filtered, static_sweep_dir, key="staticRwndBytes", key_modifier=lambda x: x / 1e3, xlabel="RWND clamp (KB)")
graph_avg_tput(static_sweep_exp_to_data_filtered, static_sweep_dir, key="staticRwndBytes", key_modifier=lambda x: x / 1e3, xlabel="RWND clamp (KB)")
...
def graph_queue(exp_to_data, sweep_dir):
    ...
    fig, axes = analysis.get_axes(rows=1, cols=1)
    ax = axes[0]

    # Plot a line for each marking threshold.
    max_x = 0
    max_y = 0
    for i, (exp, data) in enumerate(reversed(sorted(exp_to_data.items(), key=lambda p: p[1]['config']['staticRwndBytes']))):
    ...
    ax.tick_params(axis='x', labelsize=analysis.FONTSIZE)
    ax.tick_params(axis='y', labelsize=analysis.FONTSIZE)
    ax.legend(fontsize=analysis.FONTSIZE, bbox_to_anchor=(1.02, 0.5), loc="center left")
...
for conns in [50, 100, 150, 200, 500]:
    print(conns)
    graph_queue({exp: data for exp, data in static_sweep_exp_to_data.items() if data["config"]["numBurstSenders"] == conns}, static_sweep_dir)
...
def graph_fct(exp_to_data, sweep_dir):
    ...
    fig, axes = analysis.get_axes(1, width=8, height=3)
    ...
    for i, (exp, data) in enumerate(reversed(sorted(exp_to_data.items(), key=lambda p: p[1]['config']['staticRwndBytes']))):
        ...
        for burst_idx in range(1, num_bursts):
            times = [
                ...
                for flow_times_by_burst in data["sender_to_flow_times_by_burst"].values()
            ]
            durations.extend([(end - start) * 1e3 for start, _, end, _ in times])

        count, bins_count = np.histogram(durations, bins=len(durations))
        axes[0].plot(bins_count[1:], np.cumsum(count / sum(count)), COLORS_FRIENDLY_HEX[i], label=f"RWND: {round(data['config']['staticRwndBytes'] / 1024)} KB", linewidth=analysis.LINESIZE, alpha=0.7)        
    ...
    axes[0].legend(fontsize=analysis.FONTSIZE, bbox_to_anchor=(1.02, 0.5), loc="center left")

    analysis.show(fig)
    analysis.save(graph_dir, suffix=f"flow_duration_cdf")


for conns in [50, 100, 150, 200, 500]:
    print(conns)
    # graph_fct(static_sweep_exp_to_data_filtered, static_sweep_dir)
    graph_fct({exp: data for exp, data in static_sweep_exp_to_data.items() if data["config"]["numBurstSenders"] == conns}, static_sweep_dir)
...
def graph_total_cwnd(exp_to_data, sweep_dir):
    ...
    max_y = 0
    for i, (exp, data) in enumerate(reversed(sorted(exp_to_data.items(), key=lambda p: p[1]['config']['staticRwndBytes']))):
        bdp_bytes = data["config"]["smallLinkBandwidthMbps"] * 1e6 / 8 * 6 * data["config"]["delayPerLinkUs"] / 1e6
        print(bdp_bytes)
        # Last element in the tuple is the total cwnd
        xs, _, _, _, _, _, ys = copy.copy(data["cwnd_metrics_across_bursts"])
        print(ys[-10:])
        xs = np.array(xs) - xs[0]
        xs = xs * 1e3
        ys /= bdp_bytes
        ax.plot(xs, ys, COLORS_FRIENDLY_HEX[i], label=f"RWND: {round(data['config']['staticRwndBytes'] / 1024)} KB", linewidth=analysis.LINESIZE, alpha=0.7)
        max_x = max(max_x, xs[-1])
        max_y = max(max_y, max(ys))
    ...
    ax.set_xlabel("time (ms)", fontsize=analysis.FONTSIZE)
    ax.set_ylabel("total CWND (x BDP)", fontsize=analysis.FONTSIZE)
    # ax.set_xlim(left=0)
    ax.set_ylim(bottom=-1, top=max_y * 1.1)        
    ax.tick_params(axis='x', labelsize=analysis.FONTSIZE)
    ax.tick_params(axis='y', labelsize=analysis.FONTSIZE)
    ...
    plt.tight_layout()
    analysis.show(fig)
    analysis.save(graph_dir, suffix="total_cwnd")


for exp, data in static_sweep_exp_to_data.items():
    print(exp)

d = static_sweep_exp_to_data["/data_hdd/ccanel/incast/incast_sweep_static/background-senders/15ms-100-0-11-TcpDctcp-10000mbps-2000000B-10icwnd-0offset-static-rwnd8192B-20tokens-4g-80ecn-1_0da"]
...
# for conns in [50, 100, 150, 200, 500]:
#     print(conns)
#     graph_total_cwnd({exp: data for exp, data in static_sweep_exp_to_data.items() if data["config"]["numBurstSenders"] == conns}, static_sweep_dir)
